...
| PDPA — access control/security | Prevent anonymous, cross-user, unassigned-Advisor, wrong-stage, unauthorized administration, and stale booking-start access | Explicit grants, RLS, trusted role allowlist, RPC/trigger validation, 57 pgTAP assertions | Core controls implemented; latest migration deployment plus institutional-domain, MFA, suspension, and anti-abuse remediation remain open |

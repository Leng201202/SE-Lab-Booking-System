...
| PDPA — access control/security | Prevent anonymous, cross-user, unassigned-Advisor, wrong-stage, unauthorized administration, stale booking-start access, and unauthorized cancellation | Explicit grants, RLS, trusted role allowlist, RPC/trigger validation, 71 pgTAP assertions | Core controls implemented; latest migration deployment plus institutional-domain, MFA, suspension, and anti-abuse remediation remain open |

...
| Role profile (student/teacher) | Every account has a role, shown next to the holder's identifier on every booking tile. Agreed: show **student ID**, not name | Planned (app shows name today) |
| Signed-in-only booking visibility | Booking holder identity (student ID + role) visible to **signed-in users only**; signed-out visitors see only the sign-in screen | Planned (RLS currently grants anon read) |
...
| Booking detail view | Click any tile to see who booked it (holder's **student ID + role**), time range, mode, and purpose | Planned (agreed: student ID; app shows name today) |

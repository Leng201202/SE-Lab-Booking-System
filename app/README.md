...
All roles can book PCs. Student requests require Advisor then Dean approval, Advisor requests require Dean approval, and Dean requests are immediately approved when available. Every role can cancel only its own active future bookings and must give a reason. Advisors manage their advisees; Deans manage users, role assignments, and PC inventory.
